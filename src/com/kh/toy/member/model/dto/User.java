...
public class User {
	...
	//2. 자바빈 패턴
	//	getter/setter
	// 단점 : setter가 public 메서드이므로, 변경불가능한 객체(immutable)를 만들 수 없다.
	//		생성자는 getter, setter를 안만들면 되기 때문에 변경될 일이 없다.
	
	//3. 빌더패턴
	//Effective Java 책에 나온 Builder패턴
	private String userId;
	private String password;
	private String email;
	private String tell;
	
	//외부에서 User를 생성하면 안됨 (필드값에 null이 들어기기 때문, userBuilder에서만 생성해야함)
	...
	public static class UserBuilder{
		...
		public UserBuilder userId(String userId) {
			this.userId = userId;
			//자기자신을 리턴 (userId가 초기화된 UserBuilder를 리턴)
			return this;
		}
